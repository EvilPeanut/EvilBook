...
public class EventListenerBlock implements Listener {
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		if (!EvilBook.isInSurvival(player) && !EvilBook.getProfile(player).rank.isHigher(Rank.Builder) && (block.getType() == Material.ANVIL || block.getType() == Material.SAPLING || block.getType() == Material.SAND || block.getType() == Material.GRAVEL)) {
			...
			event.setCancelled(true);
		} else if (!EvilBook.getProfile(player).rank.isAdmin() && (block.getType() == Material.WATER ||
				...
				(!EvilBook.isInSurvival(player) || !EvilBook.getProfile(player).rank.isHigher(Rank.Architect))) {
			...
			if (!EvilBook.getProfile(player).rank.isAdmin() && !EvilBook.isInSurvival(player) && block.getType() == Material.ICE) block.setType(Material.PACKED_ICE);
	...
	public void onBlockIgnite(BlockIgniteEvent event) {
		...
		if (EvilBook.isInSurvival(block.getWorld())) {
